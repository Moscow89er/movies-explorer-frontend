...
function MoviesCardList ({ movies, parentComponent, onClose }) {
    return (
        <ul className="movies-cardlist">
            {movies.length === 0 ? (
                <div className="movies-cardlist__popup">
                    <button className="movies-cardlist__close-button" onClick={onClose}/>
                    <p className="movies-cardlist__error-nothing">Ничего не найдено.</p>
                </div>
            ) : (
                movies.map((movie) => {
